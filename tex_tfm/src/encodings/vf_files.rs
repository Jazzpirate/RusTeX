...
pub(crate) fn parse_vf(f:&str) -> Option<VFGlyphMap> {
    ...
    while s.0.contains("(CHARACTER") {
        ...
        'B: while !s.is_empty() {
            if s.drop(")") { break }
            if s.drop("(CHARWD") || s.drop("(CHARHT") || s.drop("(CHARDP") || s.drop("(CHARIC") ||
                s.drop("(COMMENT") || s.drop("(NEXTLARGER") || s.drop("(VARCHAR") {
                s.read_until_parens();
                continue
            }
            if s.drop("(MAP") {
                ...
                while!s.is_empty() {
                    ...
                    if s.drop("(SETRULE") || s.drop("(SPECIAL") || s.drop("(MOVEDOWN") || s.drop("(MOVERIGHT") || s.drop("(PUSH") || s.drop("(POP") {
                        s.read_until_parens();
                        continue
                    }
                    todo!()
