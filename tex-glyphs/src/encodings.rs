...
impl<S:AsRef<str>,F:FnMut(&str) -> S> FontInfoStore<S,F> {
    ...
    pub fn new(get:F) -> Self {
        ...
        Self { pdftex_map: map, enc_files: HMap::default(),pfb_files: HMap::default(), glyph_lists:PATCHED_TABLES.to_vec(),get }
    }
    ...
    pub fn display_encoding<S2:AsRef<str>>(&mut self,name:S2) -> Option<impl Display + '_> {
        if let Some(u) = self.get_glyphlist_i(&name) {
            ...
            Some(DisplayEncoding { enc, list:self.glyph_lists.get(u)? })
        } else { None }
    }
    ...
    fn get_glyphlist_i<S2:AsRef<str>>(&mut self,name:S2) -> Option<usize> {
        ...
        for (idx,v) in m.maps.into_iter().enumerate() {
            let mut gls = v.into_iter().flat_map(|(f,i)| match deps.get(f as usize).unwrap_or_else(|| unreachable!()) {
                None => vec![GlyphI::S(0)],
                Some(k) => match self.glyph_lists.get(*k).unwrap_or_else(|| unreachable!()).get(i).0 {
...
fn parse_pdftex_map(map:&mut HMap<Box<str>, FontInfo>) {
    let enc_file = std::str::from_utf8(std::process::Command::new("kpsewhich")
        .arg("pdftex.map").output().expect("kpsewhich not found!")
        .stdout.as_slice()).expect("kpsewhich failed").trim().to_string();//get("pdftex.map");
