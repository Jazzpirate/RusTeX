...
#[derive(Clone)]
pub struct Mouth<ET:EngineType>{ pub stack:Vec<TeXMouthSource<ET>> }

impl<ET:EngineType> MouthTrait<ET> for Mouth<ET> {
    fn new(memory:&mut Memory<ET>) -> Self {
        Mouth { stack:Vec::with_capacity(2097152)}
    }
    fn new_with(tks: Vec<Token<ET>>,memory:&mut Memory<ET>) -> Self {
        Mouth {stack:tks.into_iter().rev().map(|t| TeXMouthSource::Token((t, true))).collect()}
    }
    ...
    ($mouth:expr,$state:expr,$interner:expr,$label:tt => $t:ident => $f:expr) => {
        $label:loop {
            match $mouth.stack.last_mut() {
                Some(TeXMouthSource::Token(_)) => match $mouth.stack.pop() {
                    Some(TeXMouthSource::Token(($t, _))) => $f,
                    _ => unreachable!()
                },
                Some(TeXMouthSource::String(ref mut s)) => {
...
pub enum TokenSource<ET:EngineType> {
    ...
    Single{token:Token<ET>,expand:bool,next:Option<Box<TokenSource<ET>>>},
    Def(DefExpansion<ET>)
}

pub struct SimpleExpansion<ET:EngineType>(Ptr<Vec<Token<ET>>>,usize,Option<Box<TokenSource<ET>>>);
impl<ET:EngineType> SimpleExpansion<ET> {
    pub fn new(v:Ptr<Vec<Token<ET>>>) -> Self {
        Self(v,0,None)
    }
    fn get_next_simple(&mut self, interner: &mut Interner<ET::Char>, catcodes: &CategoryCodeScheme<ET::Char>, endline: Option<ET::Char>) -> Option<Token<ET>> {
        match &mut self.2 {
            None => None,
            Some(s) => s.get_next_simple(interner,catcodes,endline)
        }
    }
    fn get_next(&mut self, state: &ET::State, interner: &mut Interner<ET::Char>, outputs: &mut Outputs) -> Option<(Token<ET>, bool)> {
        todo!()
...
/*
pub struct StringMouth<ET:EngineType>(StringSource<ET::Char>,Option<Box<dyn InnerMouthIter<ET>>>,bool);
impl<ET:EngineType> StringMouth<ET> {
    pub fn new(s:StringSource<ET::Char>,next:Option<Box<dyn InnerMouthIter<ET>>>) -> Self {
        Self(s,next,false)
    }
}
impl<ET:EngineType> InnerMouthIter<ET> for StringMouth<ET> {
    fn get_next_simple(&mut self, interner: &mut Interner<ET::Char>,catcodes:&CategoryCodeScheme<ET::Char>,endline:Option<ET::Char>) -> Option<Token<ET>> {
        match self.0.get_next(interner,catcodes,endline) {
            Some(t) => Some(t),
            None => panic!("File ended unexpectedly")
        }
    }
    fn destroy(self) -> Option<Box<dyn InnerMouthIter<ET>>> {
        debug_log!(debug => "file end; inserting \\everyeof");
        match state.get_primitive_toks("everyeof") {
            None => self.1,
            Some(v) if v.is_empty() => self.1,
            Some(v) => {
                /*
                self.stack.push(TeXMouthSource::Token(eof));
                for t in v.iter().skip(1).rev() { self.stack.push(TeXMouthSource::Token((t.clone(),true))) };
        ...
        self.1.get_next(state,interner,outputs)
    }
